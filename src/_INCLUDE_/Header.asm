...
; Vector table
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
		dc.l	PORT_C_CTRL-1			; Stack pointer (set like so for initialization routine)
		
		dc.l	.PrgInit			; Entry pointer
		
		dc.l	Exception			; Bus error
		dc.l	AddressError			; Address error
		dc.l	IllegalInstr			; Illegal instruction error
		dc.l	DivideByZero			; Division by zero error
		dc.l	Exception			; CHK out of bounds error
		dc.l	Exception			; TRAPV interrupt
		dc.l	Exception			; Privilege violation error
		dc.l	Exception			; TRACE interrupt
		dc.l	Exception			; Line A emulation
		dc.l	Exception			; Line F emulation
...
.InitVals:	dc.w	$18-1				; Number of registers to set up
		...
		dc.l	H_Interrupt			; Horizontal interrupt
.VDPDPort:	dc.l	VDP_DATA			; Interrupt level 5
		dc.l	V_Interrupt			; Vertical interrupt
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
.WarmBoot:
		lea	r_Stack_Base.w,sp		; Set the stack pointer
