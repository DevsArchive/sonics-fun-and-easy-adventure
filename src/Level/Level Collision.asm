...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Player_ChkCollision:
		move.l	r_1st_Col.w,r_Col_Addr.w	; Get primary collision address
		cmpi.b	#$C,oTopSolid(a0)		; Are we on the primary path?
		beq.s	.NotPrimary			; If not, branch
		move.l	r_2nd_Col.w,r_Col_Addr.w	; Get secondary collision address
...
.NotPrimary:
		...
		beq.w	Player_HitLWall			; If so, branch
		cmpi.b	#$80,d0				; Are we hitting a ceiling?
		beq.w	Player_HitCeilAndWalls		; If so, branch
		cmpi.b	#$C0,d0				; Are we hitting a right wall?
		beq.w	Player_HitRWall			; If so, branch
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Player_HitFloorAndWalls:
		bsr.w	Player_ChkLeftWallDist		; Get left wall distance
		tst.w	d1				; Have we hit the wall?
		bpl.s	.NoLeftHit			; If not, branch
		sub.w	d1,oX(a0)			; Move out of the wall
		clr.w	oXVel(a0)			; Stop moving

.NoLeftHit:
		bsr.w	Player_ChkRightWallDist		; Get right wall distance
		tst.w	d1				; Have we hit the wall?
		bpl.s	.NoRightHit			; If not, branch
		add.w	d1,oX(a0)			; Move out of the wall
		clr.w	oXVel(a0)			; Stop moving

.NoRightHit:
		bsr.w	Player_ChkFloor			; Get floor distance
...
.TouchFloor:
		...
		add.w	d1,oY(a0)			; Move out of the floor
...
.HitFloor:
		...
		bra.w	Player_ResetOnFloor		; Reset status on floor
...
.HitSlope:
		bsr.w	Player_ResetOnFloor		; Reset status on floor
...
.ChkCeil:
		bsr.w	Player_ChkCeiling		; Get ceiling distance
		...
		add.w	d1,oY(a0)			; Move out of the ceiling
...
.NoRightHit:
		...
		tst.w	d1				; Have we hit the floor?
		bpl.s	.End				; If not, branch
		sub.w	d1,oY(a0)			; Move out of ceiling
...
.ChkCeil:
		...
		tst.w	d1				; Have we hit the ceiling?
		bpl.s	.ChkFloor			; If not, branch
		sub.w	d1,oY(a0)			; Move out of the ceiling
		tst.w	oYVel(a0)			; Are we moving up?
		bpl.s	.End				; If not, branch
		clr.w	oYVel(a0)			; Stop Y movement
...
.ChkFloor:
		...
		clr.w	oYVel(a0)			; Stop Y movement
		move.w	oXVel(a0),oGVel(a0)		; Set ground velocity
		bra.s	Player_ResetOnFloor		; Reset status on floor
...
Player_ResetOnFloor_Part2:
		...
		clr.b	oAni(a0)			; Set walking animation
		subq.w	#5,oY(a0)			; Align with floor

Player_ResetOnFloor_Part3:
...
.NotPrimary:
		...
		move.l	oX(a0),d3			; Get X position
		move.l	oY(a0),d2			; Get Y position
		move.w	oXVel(a0),d1			; Get X velocity
		ext.l	d1				; ''
		asl.l	#8,d1				; Shift it
...
.DownUp:
		...
		beq.w	Player_ChkFloorDist_Part2	; If quadrant 0, get the floor distance
		cmpi.b	#$80,d0				; Are we in quadrant $80?
		beq.w	Player_ChkCeilingDist_Part2	; If so, branch
...
.ChkWall:
		...
		beq.w	Player_ChkLeftWallDist_Part2	; If so, branch
		bra.w	Player_ChkRightWallDist_Part2	; Get the right wall distance
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
.NotPrimary:
		...
		andi.b	#$C0,d0				; Get quadrant
		cmpi.b	#$40,d0				; Are we in quadrant $40?
		beq.w	Player_ChkLeftCeilDist		; If so, branch
		cmpi.b	#$80,d0				; Are we in quadrant $80?
		beq.w	Player_ChkCeiling		; If so, branch
		cmpi.b	#$C0,d0				; Are we in quadrant $C0?
		beq.w	Player_ChkRightCeilDist		; If so, branch
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
.NotPrimary:
		...
		move.w	oY(a0),d2			; Get Y position
		move.w	oX(a0),d3			; Get X position
		moveq	#0,d0
...
Player_ChkRightCeilDist:
		...
		bra.w	Player_PickAngle		; Get the angle
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
; Get the distance between a right wall and a player object
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Player_ChkRightWallDist:
		move.w	oY(a0),d2			; Get Y position
		move.w	oX(a0),d3			; Get X position

Player_ChkRightWallDist_Part2:
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Player_ChkCeilingDist:
		move.w	oY(a0),d2			; Get Y position
		move.w	oX(a0),d3			; Get X position

Player_ChkCeilingDist_Part2:
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Player_ChkLeftWallDist:
		move.w	oY(a0),d2			; Get Y position
		move.w	oX(a0),d3			; Get X position

Player_ChkLeftWallDist_Part2:
...
.DownUp:
		...
		beq.w	Player_MoveLWall		; If so, branch
		cmpi.b	#$80,d0				; Are we in quadrant $80 (ceiling)?
		beq.w	Player_MoveCeiling		; Is so, branch
		cmpi.b	#$C0,d0				; Are we in quadrant $C0 (right wall)?
		beq.w	Player_MoveRWall		; If so, branch
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
Player_MoveFloor:
		...
		; Update the object's angle
		pop.w	d0				; Restore the primary floor distance
		bsr.w	Player_SetAngle			; Set the new angle
		...
		add.w	d1,oY(a0)			; Move us on to the surface
...
Player_MoveRWall:
		...
		cmpi.w	#-$E,d1				; Have we hit a wall?
		blt.s	.End				; If so, branch
		add.w	d1,oX(a0)			; Move us on to the surface
...
Player_MoveCeiling:
		...
		cmpi.w	#-$E,d1				; Have we hit a wall?
		blt.s	.End				; If so, branch
		sub.w	d1,oY(a0)			; Move us on to the surface
...
Player_MoveLWall:
		...
		cmpi.w	#-$E,d1				; Have we hit a wall?
		blt.s	.End				; If so, branch
		sub.w	d1,oX(a0)			; Move us on to the surface
...
.NoYFlip:
		...
		movea.l	r_Col_Array_N.w,a2		; Get the normal collision array
...
.NoXFlip:
		...
		movea.l	r_Col_Array_R.w,a2		; Get the normal collision array
		move.b	(a2,d1.w),d0			; Get height value
		ext.w	d0				; ''
		eor.w	d6,d4				; Flip the flip bits from the block
...
Level_FindBlock:
		...
		clr.w	d1				; ''
		lea	r_Layout.w,a1			; Get layout pointer
