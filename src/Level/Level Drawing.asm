...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Level_InitPlanes:
		lea	r_FG_Cam.w,a1			; Get foreground level drawing RAM
		lea	r_FG_Col_Buf.w,a3		; Get foreground column plane buffer
		lea	r_FG_Row_Buf.w,a4		; Get foreground row plane buffer
		...
		move.w	cY(a1),r_VScroll_FG.w		; Set the V-Scroll value for the foreground

		lea	r_BG_Cam.w,a1			; Get background level drawing RAM
		lea	r_BG_Col_Buf.w,a3		; Get background column plane buffer
		lea	r_BG_Row_Buf.w,a4		; Get background row plane buffer
		...
		move.w	cY(a1),r_VScroll_BG.w		; Set the V-Scroll value for the background
		rts
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
; Update the planes
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Level_UpdatePlanes:
		...
		lea	r_FG_Cam.w,a2			; Get foreground level drawing RAM
		move.w	cX(a2),cXPrev(a2)		; Update the previous X position for the foreground
		move.w	cY(a2),cYPrev(a2)		; Update the previous Y position for the foreground
		move.w	cY(a2),r_VScroll_FG.w		; Set the V-Scroll value for the foreground
		move.w	cX(a1),cXPrev(a1)		; Update the previous X position for the background
		move.w	cY(a1),cYPrev(a1)		; Update the previous Y position for the background
		move.w	cY(a1),r_VScroll_BG.w		; Set the V-Scroll value for the background
...
General_InitBG:
		lea	r_FG_Cam.w,a2			; Get foreground level drawing RAM
		move.w	cX(a2),d0			; Get foreground X position
		asr.w	#1,d0				; Divide by 2
		move.w	d0,cX(a1)			; Set as background X position
...
.DrawUp:
		move.w	cX(a1),d0			; Get X
		move.w	cY(a1),d1			; Get Y
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Level_GetRow:
		lea	r_Layout.w,a2			; Get level layout address
		adda.w	cLayout(a1),a2			; Add offset
		lea	r_Blocks.w,a3			; Get block table address
...
Level_GetCol:
		lea	r_Layout.w,a2			; Get level layout address
		adda.w	cLayout(a1),a2			; Add offset
		lea	r_Blocks.w,a4			; Get block table address
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
Level_RefreshPlane:
		move.w	cX(a1),d0			; Get X position
		...
		move.w	cY(a1),d1			; Get Y position
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
ScrollSections:
		lea	r_FG_Cam.w,a2			; Get foreground level drawing variables
		lea	r_Scrl_Secs.w,a4		; Deformation offset buffer
		lea	r_HScroll.w,a5			; Horizontal scroll buffer

		move.w	(a3)+,d0			; Get the total number of scroll sections

		move.w	cX(a2),d4			; Get camera speed
		sub.w	cXPrev(a2),d4			; ''
		ext.l	d4				; ''
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		move.w	cY(a1),d6			; Get the background's Y position
...
.FindStart:
		...
		move.w	cX(a2),d1			; Get the foreground's X position
...
ScrollStaticBG:
		...
		move.l	cX(a2),d1			; Get foreground X position
		neg.l	d1				; Negate it so it scrolls properly
		move.w	cX(a1),d1			; Get background X position
